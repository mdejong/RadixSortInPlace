...
void setupRandomPixelValues(std::vector<uint32_t> & inputValues, uint32_t maxNum) {
  ...
  //std::exponential_distribution<>  distr;
    
  for ( int i = 0 ; i < nSrcValues; i++ ) {
    uint32_t v = distr(generator);
    //uint32_t v = (uint32_t) round(rmax * distr(generator));
#if defined(DEBUG)
      assert(v <= rmax);
#endif // DEBUG
    inputValues[i] = v;
...
- (void)testCSIPTwoDigit1Opt {
  ...
  countingSortInPlaceOpt<1>(arr, 0, N);
...
- (void)testCSIPPerformanceExampleD0Opt {
  constexpr unsigned int N = PERF_N;
